...
@Composable
fun AllProductsAdmin(navController: NavHostController,razzaViewModel: RazzaViewModel = viewModel(LocalContext.current as ComponentActivity)) {
    val authViewModel =
        ...
        )
    {
        ...
        val Desending = remember { mutableStateListOf<Product>()}
        ...
        Column(modifier = Modifier.fillMaxSize()) {
            Box(
                ...
                Row(modifier = Modifier.fillMaxSize()) {
                    ...
                    Column(verticalArrangement = Arrangement.Center,
                        ...
                        SearchbarAdmin(searched,searchname)
            ...
            Box(
                ...
                ProductsScreenContentsAdmin(searched,currentCategory,navController,products)
            }
        }
    }
}
...
@Composable
fun ProductsScreenContentsAdmin(
    searched: SnapshotStateList<Product>,
        ...
        LazyColumn {
            items(searched)
            {
                ...
                if (it.category == currentCategory.id) {
                    ProductCardAdmin(it, navController)
                }
            }
        }
    }
...
fun addProduct(navController: NavHostController) {
    FloatingActionButton(
        modifier = Modifier
            ...
            .size(40.dp),
        onClick = { navController.navigate(NavScreen.AddProduct.route) },
...
@Composable
fun ProductCardAdmin(product: Product, navController : NavController,
                razzaViewModel: RazzaViewModel = viewModel(LocalContext.current as ComponentActivity)
) {
    ...
    Row {
        ...
        TextButton(
            ...
            modifier = Modifier
                .width(150.dp)
                .height(40.dp),
            onClick = { var flag: Boolean = true
                for (i in shoppingCartItems) {
                    if (i.productId.equals(product.id) && i.userEmail == authViewModel.user?.email) {
                        flag = false
                    }
                }
                if (flag) {
        ...
        TextButton(
            ...
            onClick = {
                ...
                for (i in wishListItems) {
                    if (i.pid.equals( product.id) && i.userEmail == authViewModel.user?.email) {
                        flag = false
                    }
                }
                if (flag) {
