...
@Composable
fun MyOrders1(navController: NavHostController,
...
//            //MyOrders(orders, navController)
//        orderlayout(orders, navController)
        val status = remember { mutableStateListOf<Order>() }

        val orders: List<Order> by razzaViewModel.ordersFB.collectAsState()
        orderlayout(status,orders, navController)


    }
}
@Composable
fun orderlayout(status: SnapshotStateList<Order>,
    ...
    Column(
        ...
        MyOrders(status,orders, navController)
    }
...
fun MyOrders(status: SnapshotStateList<Order>,orders: List<Order>, navController: NavHostController,
    ...
    for(o in orders){
        if(o.userEmail==authViewModel.user?.email){
            count++
        }
    }
...
fun OrderCard(order: Order, navController : NavHostController,
    ...
    if(order.userEmail==authViewModel.user?.email) {
